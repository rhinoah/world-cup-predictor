...
from teams import es                    # padron unico de las 48 selecciones
from theme import (ACCENT, ACCENT2, BAD, BG, CARD, CARD2, F, GOOD, MID, SUB,
                   TXT, WARN)
from ui_bracket import BracketMixin     # ventanas de grupos y de llaves
from ui_tray import TrayMixin           # bandeja, bips e instancia unica
...
if __name__ == "__main__":
    if sys.platform == "win32":
        try:
            ...
            pass
    _srv = check_single_instance()
    if _srv is None:
        sys.exit(0)            # ya hay una instancia viva; le pedimos que se muestre
    try:
        app = App(_srv if _srv else None)   # _srv == False -> puerto colgado, abrir sin lock
